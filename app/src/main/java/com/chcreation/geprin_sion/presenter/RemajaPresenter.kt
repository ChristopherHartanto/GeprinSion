...
class RemajaPresenter(private val view: MainView, private val auth: FirebaseAuth, private val database: DatabaseReference, private val context: Context){
    ...
    fun createRemaja(callback: TransactionInterface,remaja: Remaja){
        try {
    ...
    fun deleteRemaja(remajaId:String, callback: (success: Boolean) ->Unit){
        try{
            postListener = object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    database.removeEventListener(this)
    ...
    fun retrieveRemajaList(){
        ...
        database.child(getSinode())
            .child(getPost())
            .child(ETable.REMAJA.toString())
            .orderByChild(ERemaja.NAMA.toString())
            .addListenerForSingleValueEvent(postListener)
    }
    ...
    fun retrieveRemajaByKey(key:String){
        try{
            postListener = object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    database.removeEventListener(this)
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    view.loadData(p0, EMessageResult.FETCH_REMAJA_BY_KEY_SUCCESS.toString())
                }
